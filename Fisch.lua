...
local rodsFolder = ReplicatedStorage.resources.items.rods
...
local Tabs = { -- https://lucide.dev/icons/
    ...
    Exclusives = Window:AddTab({ Title = "Dupe", Icon = "crown" }),
    Test = Window:AddTab({ Title = "Experiment", Icon = "beaker" })
}
...
do
    Tabs.Home:AddButton({
        ...
        Callback = function()
            setclipboard("https://discord.gg/Mw4uAN2CHx")
        end
    })
    
    -- // Test Area // --
    local TEXPLA = Tabs.Test:AddSection("This is just all test idk if works")
    local Rod1 = nil
    local Rod2 = nil
    local rodNames = {}
    ...
    table.sort(rodNames)
    local Rod1D = Tabs.Test:AddDropdown("Rod1D", {
        Title = "First Rod",
        Values = rodNames,
        Multi = false,
        Default = 1,
    ...
    Rod1D:OnChanged(function(Value)
        print("First Rod:", Value)
        Rod1 = Value
    end)
    local Rod2D = Tabs.Test:AddDropdown("Rod2D", {
        Title = "Second Rod",
        Values = rodNames,
        Multi = false,
        Default = 1,
    ...
    Rod1D:OnChanged(function(Value)
        print("Second Rod:", Value)
        Rod2 = Value
    end)
    local DupeTest = Tabs.Test:AddToggle("DupeTest", {Title = "Spam", Default = false })
    DupeTest:OnChanged(function()
        while Options.DupeTest.Value do
            local args1 = {[1] = Rod1}
            print("Equiping:", Rod1)
            ...
            task.wait(DupeTestDelay)
        end
    end)
    local DupeTest_Delay = Tabs.Test:AddSlider("DupeTest_Delay", {
        Title = "Spam Delay",
        Description = "",
        Default = 0.2,
        Min = 0,
    ...
    -- // Exclusives Tab // --
    local sectionExclus = Tabs.Exclusives:AddSection("Dupe Features")
    Tabs.Exclusives:AddButton({
        Title = "Teleport to 1.06 Server (REQUIRED)",
        Description = "",
        Callback = function()
    ...
    local SAT = Tabs.Items:AddToggle("SAT", {Title = "Toggle Sell All", Default = false })
    local fishCount = 100000
    
    local Slider = Tabs.Items:AddSlider("Count", {
        Title = "Count",
        Description = "Sell all every:",
        Default = 100,
        Min = 50,
        Max = 5000,
        Rounding = 1,
    ...
    SAT:OnChanged(function()
        if Options.SAT.Value == true then
            local backpackItemCount = #LocalPlayer.Backpack:GetChildren()
            if backpackItemCount >= fishCount then
                SellAll()
            end
        end
    end)
    ...
    IdentityHiderUI:OnChanged(function()
        if Options.IdentityHiderUI.Value == true then
            ...
            -- Continuously check until turned off
            while Options.IdentityHiderUI.Value == true do
                task.wait(0.01)
            end
        else
            -- Restore original values
            if UserPlayer:FindFirstChild("streak") and originalValues.streak then
                UserPlayer.streak.Text = originalValues.streak
            end
            if UserPlayer:FindFirstChild("level") and originalValues.level then
                UserPlayer.level.Text = originalValues.level
            end
            if UserPlayer:FindFirstChild("user") and originalValues.user then
                UserPlayer.user.Text = originalValues.user
            end
            local hud = LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("hud"):WaitForChild("safezone")
            if hud:FindFirstChild("coins") and originalValues.coins then
                hud.coins.Text = originalValues.coins
            end
            if hud:FindFirstChild("lvl") and originalValues.lvl then
